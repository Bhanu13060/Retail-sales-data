...
<body>
{/*  STITCH_SHADER_START:ANIMATION_8 className="fixed inset-0 w-full h-full"  */}
...
void main() {
    ...
    // Create liquid purple motion
    for(float i = 1.0; i < 4.0; i++){
        p.x += 0.3 / i * sin(i * 3.0 * p.y + t);
        p.y += 0.3 / i * cos(i * 3.0 * p.x + t);
    }

    vec3 color1 = vec3(0.388, 0.4, 0.945); // Electric Purple
    vec3 color2 = vec3(0.1, 0.05, 0.3);    // Deep Dark Purple
    
    float noise = 0.5 + 0.5 * sin(p.x + p.y);
    vec3 finalColor = mix(color1, color2, noise * 0.5);
    
    // Add a slight glow
    finalColor += 0.1 / length(p);
